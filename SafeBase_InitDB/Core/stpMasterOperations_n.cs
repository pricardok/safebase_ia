...
{  
    ...
    [Microsoft.SqlServer.Server.SqlProcedure]
    public static void stpStartBackupDBMultiFile(string BackupType, int type = 1, int NumberOfFiles = 1)
    {
        string OperationUID = Guid.NewGuid().ToString();
        string TypeXML_JSON = "";
        string JSON_Message = "";
        ...
        SendMessage.PostLogMSG(BackupType + "_MultiFile", OperationUID, Message, XMLMessage, false);

        Message = "Finished.";

        // Validação do NumberOfFiles
        if (NumberOfFiles < 1) NumberOfFiles = 1;
        if (NumberOfFiles > 64) NumberOfFiles = 64;
        ...
        if (BackupType == "Help")
        {
            ...
            NOVA Stored Procedure com suporte a múltiplos arquivos de backup!
            ...
            Parâmetros:
            - BackupType: 'BackupFull', 'BackupDifferential', 'BackupLog'
            - type: 1, 2, 3, 4 (mesmo comportamento do stpStartBackupDB original)
            - NumberOfFiles: Número de arquivos para dividir o backup (1 a 64)

            Exemplos:
            -- Backup Full com 4 arquivos
            EXECUTE [dbo].[stpStartBackupDBMultiFile] 'BackupFull', 1, 4;

            -- Backup Differential com 2 arquivos
            EXECUTE [dbo].[stpStartBackupDBMultiFile] 'BackupDifferential', 2, 2;

            -- Backup Log com 1 arquivo (comportamento padrão)
            EXECUTE [dbo].[stpStartBackupDBMultiFile] 'BackupLog', 1, 1;

            OBS: Para restaurar backups com múltiplos arquivos, use:
            ...
            try
            {
                ...
                foreach (DataRow row in Databases.Rows)
                {
                    ...
                    else
                    {
                        ...
                        // Construir comando de backup com múltiplos arquivos
                        string backupCommand = "";
                        
                        switch (BackupType)
                        {
                            case "BackupFull":
                                backupCommand = "BACKUP DATABASE [" + DBName + "] TO ";
                                break;
                            case "BackupDifferential":
                                backupCommand = "BACKUP DATABASE [" + DBName + "] TO ";
                                break;
                            case "BackupLog":
                                backupCommand = "BACKUP LOG [" + DBName + "] TO ";
                                break;
                        }

                        // Adicionar os múltiplos arquivos
                        for (int i = 1; i <= NumberOfFiles; i++)
                        {
                            ...
                            string fileNumber = NumberOfFiles > 1 ? $"_PART{i:00}" : "";
                            string filePath = FilePath + "\\" + DBName + "_" + 
                                            (BackupType == "BackupFull" ? "FULL" : 
                                             BackupType == "BackupDifferential" ? "DIFF" : "LOG") + 
                                            "_" + Time + fileNumber + fileExtension;
                            
                            backupCommand += "DISK='" + filePath + "'";
                            if (i < NumberOfFiles) backupCommand += ", ";
                        }

                        backupCommand += " " + WITH;

                        // Adicionar delete conforme o tipo
                        if (type == 1)
                        {
                            switch (BackupType)
                            {
                                case "BackupFull":
                                    backupCommand += "; EXECUTE [dbo].[stpStartDeleteOldBackups] @BackupType = 'BackupFull';";
                                    break;
                                case "BackupDifferential":
                                    backupCommand += "; EXECUTE [dbo].[stpStartDeleteOldBackups] @BackupType = 'BackupDifferential';";
                                    break;
                                case "BackupLog":
                                    backupCommand += "; EXECUTE [dbo].[stpStartDeleteOldBackups] @BackupType = 'BackupLog';";
                                    break;
                            }
                        }
                        else if (type == 2)
                        {
                            ...
                            {
                                case "BackupFull":
                                    backupCommand += "; EXECUTE [dbo].[stpStartDeleteBackupsCustom] 'BackupFull', '" + DBName + "';";
                                    break;
                            }
                        }
                        else if (type == 3)
                        {
                            ...
                            {
                                case "BackupFull":
                                    backupCommand = "EXECUTE [dbo].[stpStartDeleteBackupsCustom] 'BackupFull', '" + DBName + "'; " + backupCommand;
                                    break;
                            }
                        }

                        scriptLine = backupCommand;

                        try
                        {
                            if (NumberOfFiles > 1)
                            {
                                SendMessage.PostBack($"Database: {DBName} - Iniciando backup com {NumberOfFiles} arquivos");
                            }
                            
                            ExecuteSql.NonQuery(OperationUID, scriptLine, true, false, true);
                            DBMessage = "Succeeded" + (NumberOfFiles > 1 ? $" (Files: {NumberOfFiles})" : "");
                            
                            if (NumberOfFiles > 1)
                            {
                                SendMessage.PostBack($"Database: {DBName} - Backup concluído com sucesso em {NumberOfFiles} arquivos");
                            }
                        }
                        catch (Exception ex)
                        {
